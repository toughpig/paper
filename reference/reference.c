...
2.2.2 模拟驱动方式
体系结构模拟器的驱动方式主要有两种：执行驱动（又叫程序驱动）和踪迹驱动。
图 2.5 两种执行驱动模拟策略
对于执行驱动模拟，存在两种不同的观点：一种认为执行驱动模拟器输入的是程序的二进制文件（可执行指令），而不是踪迹信息，因此，模拟器的输入集大小只与程序的静态指令数相关，而与动态指令数无关。如图 2.5(a)所示，由模拟器来执行程序的所有可执行指令。还有一种观点认为，执行驱动模拟器的输入信息（程序的可执行指令）分成两部分：一部分由宿主机代为执行，另一部分则由模拟器执行。如图 2.5(b)所示，这类模拟器只执行与所研究问题相关的指令，其它指令则由宿主机代替执行。这样做的优点在于模拟速度较快，但是这种模拟方式要求宿主机与目标机的指令集体系结构一致，这就限制了它的应用范围。
